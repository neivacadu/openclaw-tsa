...
async function pushMetrics(cfg: RankerConfig): Promise<void> {
...
// ============================================================================
// Hooks (OpenClaw plugin contract)
// ============================================================================

interface PromptBuildContext {
  systemPrompt: { vars: Record<string, string> };
...
function getRanker(cfg?: Partial<RankerConfig>): SkillsRanker {
  if (!_ranker) _ranker = new SkillsRanker(cfg);
  return _ranker;
}

export async function onBeforePromptBuild(ctx: PromptBuildContext): Promise<void> {
  const r = getRanker(ctx.config);
  const lastUser = [...ctx.messages].reverse().find((m) => m.role === "user");
  const query = lastUser?.content ?? "";
  const top = r.rank(query, r["cfg"].topN);
  const snapshot = r.buildSnapshot(top);
  const key = r["cfg"].snapshotKey;
  ctx.systemPrompt.vars[key] = snapshot;
  await pushMetrics(r["cfg"]);
}

export async function onAfterToolCall(ctx: ToolCallContext): Promise<void> {
  if (ctx.tool.name !== "skill_invoke") return;
  const r = getRanker(ctx.config);
  const skillId = String((ctx.tool.input as Record<string, unknown> | undefined)?.skill_id ?? "");
  if (!skillId) return;
  const result = (ctx.tool.output as { ok?: boolean })?.ok === false ? "error" : "ok";
  r.recordInvocation(skillId, ctx.sessionId ?? null, null, result);
  await pushMetrics(r["cfg"]);
}

export async function toolSkillSearch(
  ctx: ToolContext,
): Promise<{
  skills: Array<{ skill_id: string; name: string; description: string; invocation_hint: string }>;
}> {
  const r = getRanker(ctx.config);
  const limit = Math.min(50, Math.max(1, ctx.input.limit ?? 10));
  const results = r.search(ctx.input.query, limit);
  return {
