...
# SDK refactor: rodamos via JS compilado (dist/src/skills-ranker.js).
# Pre-req: rodar 'npm install && npm run build' antes desse script.
COMPILED="$PLUGIN_DIR/dist/src/skills-ranker.js"
if [[ ! -f "$COMPILED" ]]; then
  log "ERRO: nao achei $COMPILED. Rode 'npm install && npm run build' antes."
  exit 3
fi

set +e
"$NODE_BIN" --input-type=module -e "
  import { SkillsRanker } from '$COMPILED';
